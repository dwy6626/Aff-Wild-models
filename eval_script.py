...
tf.app.flags.DEFINE_integer(
    'seq_length', 80, 'the sequence length: how many consecutive frames to use for the RNN; if the network is only CNN then put here any number you want : total_batch_size = batch_size * seq_length')

tf.app.flags.DEFINE_integer(
    'size', 96, 'dimensions of input images, e.g. 96x96')
...
tf.app.flags.DEFINE_string('pretrained_model_checkpoint_path', './weights/vggface/best_model_4096x2000x2/4096x4096x2/model.ckpt-975',
                           '''the pretrained model checkpoint path to restore,if there exists one  '''
                           '''''')
...
def evaluate():
    ...
    with g.as_default():
        ...
        input_queue = tf.train.slice_input_producer(
            [images, labels, images], num_epochs=None, shuffle=False, seed=None, capacity=1000, shared_name=None, name=None)
        ...
        if FLAGS.network == 'vggface_4096':
            ...
            with slim.arg_scope(resnet_v1.resnet_arg_scope()):
                net, _ = resnet_v1.resnet_v1_50(
                    inputs=images_batch, is_training=False, num_classes=None)
            ...
            with tf.variable_scope('rnn') as scope:
                ...
                outputs = tf.reshape(
                    outputs, (FLAGS.batch_size * FLAGS.sequence_length, 128))
