            ...
            if layer[0] == 'conv':
                with tf.variable_scope(name) as scope:
                    ...
                    kernel = self.make_var(
                        'weights', shape=[h, w, c_i, c_o], trainable=True)
                    conv = tf.nn.conv2d(self.get_output(), kernel, [
                                        1]*4, padding='SAME')
                    biases = self.make_var('biases', [c_o], trainable=True)
                    bias = tf.reshape(tf.nn.bias_add(
                        conv, biases), conv.get_shape().as_list())
                ...
                with tf.variable_scope(name) as scope:
                    ...
                    weights = self.make_var(
                        'weights', shape=[dim, num_out], trainable=True)
                ...
                with tf.variable_scope(name) as scope:
                    ...
                    outputs, _ = tf.nn.dynamic_rnn(
                        cell, feed_in, dtype=tf.float32)
                    net = tf.reshape(outputs, (self.batch_size, num_units))

                    weights = self.make_var('weights_rnn', shape=[
                                            num_units, num_out], trainable=True)
                    biases = self.make_var(
                        'biases_rnn', [num_out], trainable=True)
                    op = tf.nn.relu_layer if relu else tf.nn.xw_plus_b
